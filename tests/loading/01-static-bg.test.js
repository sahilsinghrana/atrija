...
describe('Loading Optimization: Static Background', () => {
  it('BaseLayout has inline gradient background in head style block', () => {
    const layout = readFileSync(join(process.cwd(), 'src/layouts/BaseLayout.astro'), 'utf-8');
    // The <style> block in <head> should set a gradient background on body
  ...
  it('BaseLayout head has preload for loader.css and main.css', () => {
    const layout = readFileSync(join(process.cwd(), 'src/layouts/BaseLayout.astro'), 'utf-8');
    const headMatch = layout.match(/<head>([\s\S]*?)<\/head>/);
    const headContent = headMatch[1];
    // CSS preloads should exist for critical stylesheets
    expect(headContent).toMatch(/rel="preload"[^>]*href="\/css\/loader\.css"/);
    expect(headContent).toMatch(/rel="preload"[^>]*href="\/css\/main\.css"/);
  });
});
